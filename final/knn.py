...
class KNN:
    ...
    def compute_distances_two_loops(self, X):
        ...
        if self.metric == 'manhattan':
            ...
            for i_test in range(num_test):
                for i_train in range(num_train):
                    # TODO: Fill dists[i_test][i_train]
                    dists[i_test][i_train] = np.sum(square(self.train_X[i_train] - X[i_test]))
                    #pass
            return dists
    ...
    def compute_distances_one_loop(self, X):
        ...
        if self.metric == 'manhattan':
            for i_test in range(num_test):
                ...
                dists[i_test, :] = np.sum(np.abs(X[i_test] - self.train_X), axis=1)# without additional loops or list comprehensions
                #pass
            return dists
        elif self.metric == 'euclidean':
            for i_test in range(num_test):
                # TODO: Fill the whole row of dists[i_test]
                dists[i_test, :] = np.sum(np.square(X[i_test] - self.train_X), axis=1)# without additional loops or list comprehensions
                #pass
            return dists
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            ...
            nearest = self.train_y[k_nearest_idxs]
            pred[i] = np.argmax(np.bincount(nearest))
